...
const sendOtpRegister = async (data) => {
...
const confirmOtpRegister = async (email, otp) => {
  ...
  if (selectedPage === "register" && validateRegister()) {
    try {
      ...
      if (res.success) {
        ...
        // Bạn có thể chuyển sang trang nhập OTP nếu muốn
        // Hoặc xác nhận luôn ở đây nếu đã tích hợp confirmOtpRegister
        setSelectedPage("login");
      } else {
    ...
    try {
      ...
      if (res.token) {
        localStorage.setItem("token", res.token); // hoặc lưu thông tin khác nếu cần
