...
def seed_user():
    try:
        # Lấy vai trò
        admin_role = VaiTro.query.filter_by(MaVaiTro="2").first()
        khachhang_role = VaiTro.query.filter_by(MaVaiTro="1").first()

        if not admin_role or not khachhang_role:
        ...
        roles = [admin_role.MaVaiTro, khachhang_role.MaVaiTro]
